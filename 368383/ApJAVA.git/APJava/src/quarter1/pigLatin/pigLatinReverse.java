...
public class pigLatinReverse {

	public static void main(String[] args) {
		while (1 == 1) {
			System.out.println("PLEASE TYPE IN YOUR TEXT. TYPE IN 'S' TO STOP PROGRAM ");
			String original = prompt();
			String owl = firstPunct(original);
			String duck = secondPunct(outputCaps(capital(owl), process(owl)), original);
			if (duck.equals("s")) {
				System.out.println("PROGRAM HAS FINISHED ");
				break;
			} 
			String reduced = duck.substring(0, duck.length() - 1);
			String ending = duck.substring(duck.length() - 1);
			boolean whatever = false;
			if (!duck.equals("s")&&reduced.substring(0, 1).equals(duck.substring(0, 1).toUpperCase())) {
				whatever = true;
			}
			if (ending.equals("!") || ending.equals("?") || ending.equals(".")) {
				owl = duck.substring(duck.length() - 1);
			} else {
				owl = "";
			}
			String reverse = "";
			for (int i = 0; i < duck.length() - 1; i++) {
				reverse = reduced.substring(i, i + 1) + reverse;
			}
			String fox = reverse + owl;
			if (whatever == true) {
				fox = fox.substring(0, 1).toUpperCase() + fox.substring(1).toLowerCase();
			}

 {
				System.out.println("Compeletion");
